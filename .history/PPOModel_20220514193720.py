...
class AgentPPO:
    def __init__(self, net, if_explore=False):
        ...
        if if_explore:
            self.act = deepcopy(net.act).to("cpu")
            self.cri = deepcopy(net.cri).to("cpu")
        else:
            self.act = deepcopy(net.act).to(net.device)
            self.cri = deepcopy(net.cri).to(net.device)
