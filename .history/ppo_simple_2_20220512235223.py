...
class RolloutBuffer:
	...
	def sample(self, batch_size: int, continuous: bool = True):
		...
		if continuous:
			rand = random.randint(0, len(self.states) - batch_size)
			return [self.states[i] for i in range(rand, rand + batch_size)],\
					[self.actions[i] for i in range(rand, rand + batch_size)],\
					[self.logprobs[i] for i in range(rand, rand + batch_size)],\
					[self.advs[i] for i in range(rand, rand + batch_size)],\
					[self.rets[i] for i in range(rand, rand + batch_size)]
				

		else:
			indexes = np.random.choice(np.arange(len(self.states)), size=batch_size, replace=False)
			return [self.states[i] for i in indexes]
...
class PPO:
	...
	def update(self,next_value):
		...
		advs_temp=torch.squeeze(torch.stack(advs_temp, dim=0)).detach().to(device)
		advs_temp=(advs_temp-advs_temp.mean())/(advs_temp.std()+1e-7)
		self.buffer.advs=list(reversed(advs_temp))
		"""
			if don:
				discounted_reward = 0
			discounted_reward = rew + (self.gamma * discounted_reward)
			rets_temp.insert(0, discounted_reward)
		rewards = torch.tensor(rets_temp, dtype=torch.float32).to(device)
		rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-7)
		"""
		# """
		...
		for _ in range(self.K_epochs):
			states, actions, old_logprobs, advs, rets = self.buffer.sample(batch_size=50)
			states = torch.squeeze(torch.stack(states, dim=0)).detach().to(device)
			actions = torch.squeeze(torch.stack(actions, dim=0)).detach().to(device)
			old_logprobs = torch.squeeze(torch.stack(old_logprobs, dim=0)).detach().to(device)
			advs = torch.squeeze(torch.stack(advs, dim=0)).detach().to(device)
			rets = torch.squeeze(torch.stack(rets, dim=0)).detach().to(device)
