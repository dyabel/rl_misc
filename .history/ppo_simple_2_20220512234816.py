...
class PPO:
	...
	def update(self,next_value):
		...
		for _ in range(self.K_epochs):
			states, actions, old_logprobs, advs = self.buffer.sample(batch_size=50)
			...
			loss = -torch.min(surr1, surr2).mean() + 0.5 * self.MseLoss(values, rewards) - 0.01 * dist_entropy.mean()
			# print(values,rets)
