...
class PPO:
    ...
    def update(self,next_value):
        ...
        for i, val in enumerate(reversed(self.buffer.vals)):
            ret = value_previous
            if i == 0:
                for j in range(max_len):
                    ret = self.gamma * ret * (1-self.buffer.dones[max_len-1-i-j]) + self.buffer.rewards[max_len-1-i-j]
                    rets_temp.append(ret)
            else:
                for j in range(max_len):
                    ret = self.gamma * ret + self.buffer.rewards[max_len-1-i-j]
                rets_temp.append(ret)
